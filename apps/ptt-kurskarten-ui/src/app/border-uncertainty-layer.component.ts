...
@Component({
  ...
  template: `
    ...
    >
      <defs>
        <path [attr.id]="pathId" [attr.d]="pathD"></path>
        <clipPath [attr.id]="clipId">
          <path
            ...
            [attr.stroke-width]="resolvedConfig.zoneWidthPx"
          ></path>
        </clipPath>
      </defs>
...
export class BorderUncertaintyLayerComponent implements OnInit, OnChanges {
  ...
  private readonly instanceId = ++NEXT_LAYER_ID;

  @Input() pathD = DEFAULT_SWITZERLAND_PATH_D;
  @Input() config: Partial<BorderUncertaintyConfig> = {};
  @Input() mapTransform = '';

  readonly pathId = `ch-border-path-${this.instanceId}`;
  readonly clipId = `ch-border-zone-clip-${this.instanceId}`;
    ...
    for (let i = 0; i < cfg.bandsCount; i += 1) {
      ...
      for (const sign of signs) {
        for (let k = 0; k < attemptsPerBand; k += 1) {
          ...
          const target = (i + 1) * cfg.bandWidthPx + randomRange(rng, -cfg.bandJitterPx, cfg.bandJitterPx);
          ...
          bandDots.push({
            ...
            o: clamp(randomRange(rng, 0.35, 0.85) * Math.pow(0.9, i), 0.04, 1)
          });
        }
      }
    }
