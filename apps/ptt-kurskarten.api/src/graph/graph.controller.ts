...
export class GraphController {
  ...
  async createAssertion(@Body() body: GraphAssertion): Promise<GraphAssertion> {
    if (!body?.targetType || !body?.targetId || !body?.schemaKey) {
      throw new BadRequestException(
        'targetType, targetId and schemaKey are required',
      );
    }
  ...
  async updateAssertion(
    ...
    @Body() body: Partial<GraphAssertion>,
  ): Promise<GraphAssertion> {
    const updated = await this.graphRepository.updateAssertion(
      assertionId,
      body,
    );
    if (!updated) {
  ...
  async updateNodeVisibility(
    ...
    const updated = await this.graphRepository.setNodeHidden(
      nodeId,
      parsedYear,
      hidden,
    );
  ...
  async updateEdition(
    ...
    @Body() body: Partial<EditionEntry>,
